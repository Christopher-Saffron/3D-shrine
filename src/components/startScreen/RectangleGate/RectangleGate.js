...
export default function Model(props) {
  ...
  const handleClick = () => {
    ...
    document
      .querySelector(".startScreen__navbar")
      .classList.add("startScreen__navbar_hide");

    //// MAKE BACKGROUND WHITE TOO
    const whiteBack = document
      .querySelector(".whiteBack")
      .classList.add("whiteBackShow");
    // whiteBack.classList.add('whiteBackShow');

    // MAKE BUTTON TEXT BLEND AND DISAPPEAR
    document
      .querySelector(".startScreen__button")
      .classList.add("startScreen__button_animate");
  ...
  const meshShow = (val) => {
    ...
    if (val) {
      apiHover.start({
        opacityForWireframe: 0.7,
        emisWireframe: 2,
        opacityForBackScreen: 0.01,
        opacityForWalls: 0.3,
        emisWalls: 0.2,
        emisForBackScreen: 1,
      });
        ...
        <group name="Armature" position={[0, 0, 4.13]} rotation={[0, 0, 0]}>
          ...
          <group name="forExport">
            ...
            <skinnedMesh
              ...
              material={materials.frameInvisibleAtFirst}
              skeleton={nodes.Plane006_1.skeleton}
            >
              <meshStandardMaterial
                ref={gsapFrameInvisibleAtFirst}
                emissive={"#00A8FF"}
                emissiveIntensity={1.7}
                transparent
                opacity={0}
              ...
              <animated.meshStandardMaterial
                ref={gsapFrameInvisibleAtFirst}
                emissive={"#00A8FF"}
                emissiveIntensity={hoverSprings.emisWireframe}
                transparent
                roughness={0}
                radius={2}
            ...
            <skinnedMesh
              ...
              skeleton={nodes.Plane006_2.skeleton}
            >
              <animated.meshStandardMaterial
                ref={gsapEkran}
                emissive={"#fff"}
              ...
              {/* ================= HTML EKRAN TUTAJ */}
              <Html className="noEvents" position={[0, 0.05, -0.3]} center>
                <div>
                  <StartScreenButton
                    hoverSprings={hoverSprings}
                    ref={gsapText}
                  />
                </div>
              </Html>
            ...
            <skinnedMesh
              ...
              skeleton={nodes.Plane006_3.skeleton}
            >
              <animated.meshStandardMaterial
                ref={gsapWalls}
                emissive={"#FFF"}
                emissiveIntensity={hoverSprings.emisWalls}
                transparent
